...
function ActivityTab(){
  const{completeTab}=useGame();
  const[activeRound,setActiveRound]=useState('roles');
  const qs=[
  ...
  return <div style={{paddingTop:56}}>
    <Wrap>
      ...
      <Reveal delay={0.1}><Card style={{marginBottom:20,borderLeft:`4px solid ${C.amber}`}}>
        ...
        <P mb={10}>A generic paper summary is not enough. Every claim about the results must be tied to a specific place in the paper.</P>
        <Li><strong>For numbers:</strong> name the table, row, column and cell.</Li>
      ...
      <Reveal delay={0.16}><Card style={{marginTop:20,marginBottom:20,borderLeft:`4px solid ${C.blue}`}}>
        ...
        <div style={{padding:14,borderRadius:8,background:C.white,border:`1px solid ${C.black10}`,borderLeft:`4px solid ${C.blue}`}}>
          <div style={{fontSize:18,fontWeight:900,color:C.black,marginBottom:4}}>{selectedRound.title}</div>
          <div style={{fontSize:14,color:C.black80,lineHeight:1.6,marginBottom:10}}>{selectedRound.body}</div>
          <div style={{fontSize:13,color:C.black80,lineHeight:1.55,background:C.black05,border:`1px solid ${C.black10}`,borderRadius:6,padding:'10px 12px'}}><strong>Prompt:</strong> {selectedRound.prompt}</div>
        </div>
      </Card></Reveal>
    </Wrap>
    <Wrap bg={C.black05}>
